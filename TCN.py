...
class TCN(nn.Module):
    ...
    def per_layer_time(self, input_size=[1, 15], repeated=10):
        pl_time = []
        import time
        for i in range(repeated):
            p_time = []
            x = torch.rand((1, input_size[0], input_size[1]))
            for j in range(len(self.tcn)):
                start = time.time()
                x = self.tcn.network[j](x)
    ...
    def per_layer_energy(self, input_size=[1, 15], repeated=10):
        pl_energy = []
        import time
        for i in range(repeated):
            p_energy = []
    ...
    def forward(self, x):
        ...
        output = self.linear(output).permute(0, 2, 1).double()
        return output


if __name__ == '__main__':
    net = TCN(1, 1, [4, 8, 16], kernel_size=3, dropout=0.2)
